...
class STLVectorHeap : public Heap<T> {
 ...
 public:
  void insert(T value) override { pq.push(value); }
...
class BinomialQueue : public Heap<T> {
 private:
  ...
  // Helper functions
  std::shared_ptr<Node> mergeTrees(std::shared_ptr<Node> t1,
    ...
    // 打印调试信息
    // std::cout << "Merging trees with roots " << t1->element << " and " <<
    // t2->element << std::endl;
  ...
  void linkTrees(std::vector<std::shared_ptr<Node>>& trees) {
    for (size_t i = 0; i < trees.size(); ++i) {
      if (trees[i]) {
        // 打印调试信息
        // std::cout << "Linking tree with root: " << trees[i]->element <<
        // std::endl;
 ...
 public:
  ...
  void insert(T value) override {
    ...
    // 打印调试信息
    // std::cout << "Insert complete. Current root is: " << root->element <<
    // std::endl;
  }
  ...
  T deleteMin() override {
    ...
    // 打印调试信息
    // std::cout << "Merging child trees after deleting minimum node." <<
    // std::endl;
    ...
    // 打印调试信息
    // std::cout << "DeleteMin complete. New root is: " << (root ?
    // std::to_string(root->element) : "null") << std::endl;
