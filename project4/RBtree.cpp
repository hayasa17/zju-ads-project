...
const int mod = 1e9 + 7;
// 第一个动态规划函数，时间O(n^3),空间O(n^2)
void DP1(int n) {
  // 初始化动态规划数组，大小为 (n+1) x (n+2) x 2，所有值初始化为0
  vector dp(n + 1, vector<vector<int>>(n + 2, vector<int>(2, 0)));
  // 初始状态，只有NIL的情况
  dp[0][1][1] = 1;
  // 枚举当前考虑的树的大小
  for (int i = 1; i <= n; ++i) {
  ...
  for (int i = 1; i <= n + 1; ++i) ans = (ans + dp[n][i][1]) % mod;
  cout << ans << endl;
  return;
}
// 第二个动态规划函数，时间O(n^3),空间O(n)
void DP2(int n) {
  int ans = 0;
...
/*
  ...
  若模数更为理想，可以使用NTT进一步优化至时间O(n^2 logn)
*/
int main() {
  int n;
  cin >> n;
